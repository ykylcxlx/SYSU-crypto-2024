...
void print2d(uint8_t  text[4][4],int mode=0){
    if (mode==0){
        for(int i=0;i<4;i++){
            for(int j=0;j<4;j++){
                fwrite(&text[j][i],1,1,stdout);
                //printf("%02X ",text[j][i]);
            }
...
void AddRoundKey(int round,uint8_t state[4][4],uint8_t w[]){
	...
	{							  //							  k2 k6 k10k14
		...
		{						  // 所以i行j列的下标是4*((round*4)+j)+i即16*round+4*j+i
			state[i][j]=(unsigned char)((int)state[i][j]^(int)w[16*round+4*j+i]);
...
void invSubBytes(uint8_t state[4][4]){
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
            state[i][j]=N_S_BOX[state[i][j]>>4][state[i][j]&0x0f];
        }
    }
}
...
void getChars(unsigned char *buf,int len,int pad_len,FILE *inputFile){
...
// 在本地调试时，从文件读取输入和输出
    ...
    //plain_text = new char[len];

    //chipher(op,key,iv,plain_text,true_len);
    // //密钥扩展
    uint8_t w[176];
    keyExpansion(w,key);
    // print(w,176);
    ...
    if (op == 0x01){
        //加密
        cbcEnc(w,iv,plain_text,len,chipher_text,inputFile,mode);
    }else{
        //解密
        cbcDec(w,iv,plain_text,len,chipher_text,inputFile,mode);
        // print(plain_text,len,mode);
    }
    return 0;
}