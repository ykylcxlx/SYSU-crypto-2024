...
public:
    ...
    void modR(uint32_t res[128], uint32_t a[128]);
    void divR(uint32_t res[128], uint32_t a[129]);
    void modReduction(uint32_t res[128], uint32_t a[128], uint32_t b[128]);
    void modMulMontgomery(uint32_t res[128], uint32_t a[128], uint32_t b[128]);
    void modPowMontgomery(uint32_t res[128], uint32_t a[128], uint32_t b[128]);
...
void BigInt::modPow(uint32_t res[SIZE], uint32_t a[SIZE], uint32_t b[SIZE]) {
    ...
    void BigInt::modR(uint32_t res[128], uint32_t a[128]) {
        uint32_t temp[128] = {0};
    ...
    // // 新的 divR 函数
    void BigInt::divR(uint32_t res[128], uint32_t a[129]) {
        uint32_t temp[128] = {0};
    ...
    //蒙哥马利模乘
    void BigInt::modReduction(uint32_t res[128], uint32_t a[128], uint32_t b[128]) {
        uint32_t T[128] = {0};
    ...
    // 新的模幂函数
    void BigInt::modPowMontgomery(uint32_t res[128], uint32_t a[128], uint32_t b[128]) {
        // uint32_t temp[128] = {0};
        // uint32_t base[128] = {0};
        // modMulMontgomery(temp, ONE, R2);
        // modMulMontgomery(base, a, R2);
        ...
        //     for (int j = 0; j < 32; j++) {
        //         if (b[i] & pow2[j])
        //             modMulMontgomery(temp, temp, base);

        //         modMulMontgomery(base, base, base);
        //     }
        // }

        // modMulMontgomery(res, temp, ONE);
